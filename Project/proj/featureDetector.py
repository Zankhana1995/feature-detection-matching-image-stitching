...
thresholdMatching = 0.18
windowOfPixel = 5
# for adaptive non-maximum suppression
cRobust = 0.9


# assignment 2 code file (Harris corner detector, SIFT descriptor (not with inbuilt function) and feature matching)
# to select a fixed number of interest points from each image here 500 according to MOPS paper (restrict features)
...
# offset is used to make window fo harris to apply convolution further
...
# subtract all angle from keypoint orientation, so that all angle will be in the reference of keyPoint angle.
...
def prepareHistogram(magnitude, angle, histogram):
    ...
    # discard pixel with high magnitude so that it cannot dominate orientation.
    ...
    for x, y, value in keypoints:
        ...
        for i in iteratorList:
            for j in iteratorList:
                ...
                # make orientation of 4 x4 grid. (16 cells * 8 orientation = 128 d descriptor)
...
def featureMatching(interestPoints1, interestPoints2):
    ...
    for first in range(0, a):
        ...
        #  find sum of square difference between entries of two descriptor
        ...
        # throw out matches greater then threshold
...
def improvedMatches(matches, ssdRatio, threshold, interestPoints1, interestPoints2):
    newMatches = []
    for i in np.arange(0, len(matches)):
        p1 = interestPoints1[matches[i].queryIdx]
        p2 = interestPoints2[matches[i].trainIdx]
        d = math.sqrt(((p2[0] - p1[0]) ** 2) + ((p2[1] - p1[1]) ** 2))
        if d < threshold and ssdRatio[i][2] < 0.75:
